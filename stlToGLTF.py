...
# Normally additional binary file is created. 'True' for including this data in the gltf file
def stl_to_gltf(stlFile, gltfFile, combineBinaryFile):
    import struct
...
'''
    ...
    if not combineBinaryFile:
        out_bin = os.path.join(gltfFile, "out.bin")
        out_gltf = os.path.join(gltfFile, "out.gltf")
    else:
        out_bin = gltfFile
    ...
    with open(stlFile, "rb") as f:
        ...
        assert stl_assume_bytes == os.path.getsize(stlFile), "stl is not binary or ill formatted"
    ...
    print("Done! Exported to %s" % gltfFile)

if __name__ == "__main__":
    stlFile = "surface.stl"
    gltfFile = "gltfModel.gltf"
    stl_to_gltf(stlFile, gltfFile, True)
